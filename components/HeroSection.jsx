...
export default function HeroSection({ buyUrl, lang = 'fr', dictHero = {}, banner = '/banner.webp' }) {
  ...
  return (
    <section id="hero" className="grid lg:grid-cols-5 gap-8 items-center py-12">
      {/* Texte */}
      <div className="lg:col-span-3">
        ...
        <div className="mt-6 flex gap-3 flex-wrap">
          ...
          <Link
            href={`/${lang}/tokenomics`}
            className="px-5 py-2 rounded-2xl border border-white/10"
          >
            {dictHero?.tokenomics_btn ?? 'Tokenomics'}
          </Link>
        </div>
        ...
        <div className="mt-4 text-xs text-white/60">
          ...
          <span className="font-mono bg-white/5 px-2 py-1 rounded">
            {dictHero?.contract_value ?? 'SOON...'}
          </span>
        </div>
      </div>

      {/* Image (bannière à droite) */}
      <div className="lg:col-span-2 flex justify-center lg:justify-end">
        {banner && (
          <Image
            ...
            width={700}
            height={700}
            className="w-full h-auto max-h-[500px] rounded-2xl shadow-lg object-contain"
            priority
          />
        )}
      </div>
